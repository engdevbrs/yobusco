...
const Profile = () => {
        ...
        MySwal.fire({
            ...
            denyButtonText: `Cancelar`,
                ...
                if(result.isConfirmed){
                    showProgress(false)
                    Axios.put("http://3.89.109.142:3001/api/images",
                    formData,
                    {
                        headers: {
                        ...
                        if(result.status === 200){
                            ...
                            showProgress(true)
                            getAccess(token)
                            document.getElementById('photoUser').src = "http://3.89.109.142:3001" + result.data.imagePath
                        }
                    }).catch(error => {
    ...
    const deletePrevUserPhoto = () =>{
        Axios.delete('http://3.89.109.142:3001/api/images/delete/' + getPhoto)
          .then((result) => {
              if(result.status === 200){
                console.log(result);
              }
    ...
    const handleButton = (e) =>{
        ...
        if(e.textContent === "Actualizar Datos"){
            ...
            MySwal.fire({
                ...
                if (result.isConfirmed) {
                    Axios.put("http://3.89.109.142:3001/api/update-user", {newArrayValues ,'authorization' : `${token}`})
                    .then((result) => {
                        if(result.status === 200){
        ...
        Axios.post("http://3.89.109.142:3001/api/user-info", {
          ...
          .then((result) => {
              if(result.status === 200){
                    ...
                    localStorage.setItem('userPhoto', "http://3.89.109.142:3001/api/images/" + result.data[0].userPhoto)
